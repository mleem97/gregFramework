...
- Framework migration and modular structure (`FrikaMF`, `HexLabelMod`) are in place.
- Runtime hook/event bridge exists:
  - Harmony patch layer (`FrikaMF/HarmonyPatches.cs`)
  - Event IDs (`FrikaMF/EventIds.cs`)
  - Dispatch transport (`FrikaMF/EventDispatcher.cs`)
- Native plugin bridge exists (`FrikaMF/FfiBridge.cs`) with event forwarding support.
- C# API bridge exists (`FrikaMF/GameApi.cs`, `FrikaMF/GameHooks.cs`) for runtime game access.
...
1. Revalidate hook points in `FrikaMF/HarmonyPatches.cs` against `Assembly-CSharp` method signatures.
1. Compare event surface (`FrikaMF/EventIds.cs` + dispatch callsites) for added/removed gameplay events.
...
- Gregify employee replacement baseline and RGB Greg premium variant
- Available hires export for framework clients (`available-hires.json`)
...
- plugin sync central server hardening (hash verification, rollback, conflict resolution)
- 16-player multiplayer synchronization parity (state authority, anti-drift checks)
...
### 🔜 A-5 · Mac Compatibility Layer

**Stage 1 — Documentation (P1):**

- Add `MAC_SETUP.md` with CrossOver/Wine setup.
- Document Winetricks dependencies.
- Document `winhttp.dll` override.
- Document FrikaMF install paths.

**Stage 2 — Setup Script (P2):**

- Add `tools/install-mac.sh` for automated Wine prefix setup.
- Automate dependency setup via Winetricks.
- Automate FrikaMF deployment to the expected game paths.

**Not planned:**

- Native macOS runtime build
- Custom Wine/Proton fork

> Baseline: MelonLoader compatibility on macOS is treated as Wine/CrossOver-based community path.
