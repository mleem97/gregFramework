...
function* walkFiles(dir: string, exts: Set<string>): Generator<string> {
  for (const name of readdirSync(dir)) {
    ...
    if (st.isDirectory()) {
      if (name === 'node_modules' || name === 'build' || name === 'bin' || name === 'obj') continue;
...
function main(): void {
  ...
  const hits: { file: string; from: string; to: string; line: number }[] = [];
    ...
    for (const file of walkFiles(root, exts)) {
      ...
      lines.forEach((line, idx) => {
        for (const { from, to } of patterns) {
          if (line.includes(from)) hits.push({ file, from, to, line: idx + 1 });
        }
      });
    }
  }
