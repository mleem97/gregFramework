...
- Current repository snapshot contains only static asset payload (`image.png`).

Framework dependency status:

- Not applicable yet (no runtime mod entrypoint exists).
- Roadmap action: create runtime standalone entrypoint and enforce framework dependency before first release.
...
| Gregify employees | `StandaloneMods/FMF.GregifyEmployees/image.png` | partial | asset exists; runtime mod implementation pending |
